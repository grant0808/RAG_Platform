...
| 필드 | 타입 | 필수 | 규칙 |
|---|---|---:|---|
| `api_key` | string | O | 최소 8자, 응답과 로그에 원문 미반환 |
...
| 필드 | 타입 | 설명 |
|---|---|---|
| `provider` | string | `openai` 또는 `anthropic` |
...
| `status` | string | 현재 정상 완료 시 `ready` |
...
| `provider` | enum | X | `openai`; `openai`, `anthropic` |
...
- Path: `provider`는 대소문자를 정규화하며 `openai`, `anthropic`만 지원한다.
- 동일 Provider가 이미 있으면 API 키와 모델 목록을 교체한다.
- `validate_connection=true`이면 Provider Models API를 호출한 뒤 저장한다.
...
- 빈 파일, 잘못된 JSON, 텍스트 추출 실패는 HTTP 422다.
...
- 추정 비용은 OpenAI `$0.015`, Anthropic `$0.025`, cache hit `$0.001`의 고정값이다.
