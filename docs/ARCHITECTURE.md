...
| LangGraphRagWorkflow | 질문 분석, conversation history 반영 query rewrite, route, tool 선택, retrieval, rerank, context grading, fallback 결정을 graph로 실행 |
...
| WebSearchProvider | DuckDuckGo, Tavily, 또는 비활성(`none`) web fallback provider 제공 |
...
- Conversation은 기존 `chat_sessions` 테이블, message는 `chat_messages` 테이블을 사용한다. API의 `conversation_id`는 내부 `session_id`와 같은 값이며, assistant message metadata에 `route`, `selected_tool`, `sources`, `trace`, `usage`, `memory_used`, `history_count`를 저장한다.
