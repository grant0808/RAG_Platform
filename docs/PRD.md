...
현재 코드만으로 구현 완료로 볼 수 없는 기능은 사용자별 인증/권한, judge-model 기반 정식 RAGAS metric 평가, 비용 최적화 추천, 논문 간 비교/실험 결과/한계점 추출 전용 기능이다. 해당 항목은 제품 목표 또는 추가 제안 기능으로 취급한다. RAG 필요 여부 routing, web fallback, RAGAS 호환 JSON 평가 저장은 최소 동작 범위로 구현되어 있다.
