...
- OpenAIEmbeddings와 PostgreSQL+pgvector 기반 RAG
...
기본 설정은 OpenAI embedding과 PostgreSQL+pgvector vector store를 사용합니다.
...
`.env.example`의 `FOUNDRY_FAKE_LLM_ENABLED=true`는 외부 chat completion API를 호출하지 않는 결정적 로컬 모델을 활성화합니다. OpenAI embedding을 사용할 때는 `FOUNDRY_OPENAI_EMBEDDING_API_KEY` 또는 `FOUNDRY_OPENAI_ADMIN_API_KEY`를 설정해야 합니다. 실제 OpenAI 또는 Anthropic chat provider를 시험할 때는 `FOUNDRY_FAKE_LLM_ENABLED=false`로 바꾸고 Provider API로 실제 inference key를 등록합니다.

PostgreSQL 없이 빠른 로컬 smoke test를 실행해야 하면 vector store와 embedding만 로컬 구현으로 명시적으로 바꿉니다.
...
Docker Compose는 API, PostgreSQL(pgvector extension 초기화), Redis를 함께 시작합니다. fake chat model과 seed를 함께 사용하려면 먼저 컨테이너를 시작한 후 bootstrap을 실행합니다.
...
테스트 fixture는 `vector_store_provider="memory"`와 `embedding_provider="local"`을 명시해 OpenAI embedding API와 로컬 PostgreSQL/pgvector에 의존하지 않습니다.
...
- GCE 배포 전 Redis 기반 CAG cache, MinIO Adapter, migration을 추가해야 합니다.
